...
"""
function jacobian(geometry, ts; ε = 1e-6)
    T = eltype(ts)
...
"""
function differential(geometry, ts)
...
"""
function derivative(bz::Meshes.BezierCurve, t)
...
"""
function unitdirection(bz::Meshes.BezierCurve, t)
    # Parameter t restricted to domain [0,1] by definition
    if t < 0 || t > 1
        throw(DomainError(t, "b(t) is not defined for t outside [0, 1]."))
    end
