...
# GaussKronrod
function _integral(f, geometry, settings::GaussKronrod, FP::Type{T}=Float64) where {T<:AbstractFloat}
...
# GaussLegendre
function _integral(f, geometry, settings::GaussLegendre, FP::Type{T}=Float64) where {T<:AbstractFloat}
  N = Meshes.paramdim(geometry)
...
# HAdaptiveCubature
function _integral(f, geometry, settings::HAdaptiveCubature, FP::Type{T}=Float64) where {T<:AbstractFloat}
  Dim = Meshes.paramdim(geometry)
...
function _integral_1d(f, geometry, settings::GaussKronrod, FP::Type{T}=Float64) where {T<:AbstractFloat}
  integrand(t) = f(geometry(t)) * differential(geometry, [t])
  return QuadGK.quadgk(integrand, FP(0), FP(1); settings.kwargs...)[1]
end

function _integral_2d(f, geometry2d, settings::GaussKronrod, FP::Type{T}=Float64) where {T<:AbstractFloat}
...
function _integral_3d(f, geometry, settings::GaussKronrod, FP::Type{T}=Float64) where {T<:AbstractFloat}
