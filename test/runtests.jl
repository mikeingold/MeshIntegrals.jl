...
@testset "Integrate Unitful" begin
    ...
    unit_circle = BezierCurve(
        [Point(cos(t)*1.0u"m",sin(t)*1.0u"m",0.0) for t in range(0, 2pi, length=361)]
    )
    ...
    @testset "Scalar-Valued Functions" begin
        ...
        # integrate(f, ::Meshes.Segment)
        @test integrate(f, seg_ne) ≈ sqrt(2) * 1.0u"Ω"

        # integrate(f, ::Vector{Meshes.Segment})
        @test integrate(f, rect_traj_segs) ≈ 4.0u"Ω" * sqrt(2)

        # integate(f, ::Meshes.Ring)
        @test integrate(f, rect_traj_ring) ≈ 4.0u"Ω" * sqrt(2)

        # integate(f, ::Meshes.Rope)
        @test integrate(f, rect_traj_rope) ≈ 4.0u"Ω" * sqrt(2)

        # integrate(f, ::Meshes.BezierCurve)
        @test isapprox(integrate(f, unit_circle), 2.0u"Ω"*π; atol=0.15u"Ω")
    end
    ...
    @testset "Vector-Valued Functions" begin
        ...
        # integrate(f, ::Meshes.Segment)
        @test integrate(f, seg_ne) ≈ [sqrt(2), sqrt(2), sqrt(2)] .* 1.0u"Ω"

        # integrate(f, ::Vector{Meshes.Segment})
        @test integrate(f, rect_traj_segs)  ≈ 4 .* [sqrt(2), sqrt(2), sqrt(2)] .* 1.0u"Ω"

        # integate(f, ::Meshes.Ring)
        @test integrate(f, rect_traj_ring) ≈ 4 .* [sqrt(2), sqrt(2), sqrt(2)] .* 1.0u"Ω"

        # integate(f, ::Meshes.Rope)
        @test integrate(f, rect_traj_rope) ≈ 4 .* [sqrt(2), sqrt(2), sqrt(2)] .* 1.0u"Ω"

        # integrate(f, ::Meshes.BezierCurve)
        @test isapprox(integrate(f, unit_circle), [2π, 2π, 2π] .* 1.0u"Ω"; atol=0.15u"Ω")
    end
end
